  ...
  methods: {
    convertData (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < self.locationData.length; j++) {
          if (data[i].name === self.locationData[j].name) {
            let geoCoord = self.locationData[j].value
            res.push({
              name: data[i].name,
    ...
    generateMap () {
      const self = this
      var fontsize = screenSizeObj.remToPx(0.6)
      var bubblesize = screenSizeObj.remToPx(0.4)
      // var color = ['#495060', '#ffa022', '#46bee9']
      ...
      var option = {
        ...
        series: [
          ...
          },
          {
            ...
            rippleEffect: {
              period: 3,
              scale: screenSizeObj.remToPx(0.3),
              brushType: 'stroke'
            },
            zlevel: 1,
            data: [{name: 'Afghanistan', value: [216.46, 39.92, 0]},
              {name: 'Angola', value: [121.48, 31.22, 1]},
              {name: 'yuzhenghui', value: [75.46, 39.92, 0]}],
            symbolSize: bubblesize, // 设置气泡大小
            label: {
              normal: {
                position: 'bottom',
                show: false,
          ...
          },
          {
            ...
            data: [
              {fromName: 'Afghanistan', toName: 'Angola', coords: [[216.46, 39.92], [121.48, 31.22]]},
              {fromName: 'yuzhenghui', toName: 'Angola', coords: [[75.46, 39.92], [121.48, 31.22]]}
            ]
          }
        ]
      }
